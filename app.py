...
def fetch_all_classes(api_url):
    ...
    try:
        ...
        if classes:
            ...
            logging.warning("Não foi possível obter a lista de classes processuais.")
            return []
    except Exception as e:
        logging.error(f"Erro ao buscar classes processuais: {e}")
        return []
...
def save_to_csv(df, uf, classe_processual=None):
    ...
    filename = f"/home/ubuntu/consultas_datajud/prescricao_{uf}_{classe_slug}_{timestamp}.csv"
...
def consultar_uf(uf, classe_processual=None, progress_callback=None):
    """Consulta uma UF específica com o filtro de status de prescrição."""
    ...
    try:
        ...
        if not all_hits:
            return None, f"Nenhum processo encontrado para UF {uf} com status de prescrição."
        ...
        # Salvar em CSV
        csv_path = save_to_csv(df, uf, classe_processual)
...
def consultar_todas_ufs(classe_processual=None):
    """Consulta todas as UFs com o filtro de status de prescrição."""
    resultados = {}
    erros = {}
    ...
    for uf in UF_ENDPOINTS.keys():
        ...
        # Consultar a UF
        df, resultado = consultar_uf(uf, classe_processual, uf_progress_callback)
...
def main():
    st.set_page_config(
        page_title="Consulta DataJud - Prescrição",
        page_icon="⚖️",
        layout="wide"
    )
    
    st.title("Consulta de Processos com Status de Prescrição")
    st.markdown("""
    Esta aplicação consulta processos judiciais com status **"Arquivado - Extinção da Punibilidade - Prescrição"** 
    em todos os tribunais disponíveis na API pública do DataJud.
    """)
    ...
    if consulta_option == "Consultar UF específica":
        ...
        if selected_uf:
            ...
            if api_url:
                ...
                with st.spinner("Carregando classes processuais..."):
        ...
        if st.sidebar.button("Consultar"):
            if not selected_uf:
                ...
                st.subheader(f"Consultando processos com status de prescrição para UF: {selected_uf}")
                if selected_class:
                ...
                with st.spinner("Consultando API do DataJud..."):
                    ...
                    df, resultado = consultar_uf(selected_uf, selected_class, progress_callback)
        ...
        # Botão para consulta
        if st.sidebar.button("Consultar Todas as UFs"):
            # Exibir informações da consulta
            st.subheader("Consultando processos com status de prescrição para todas as UFs")
            if selected_class:
                st.info(f"Filtro adicional por classe processual: {selected_class}")
            else:
                st.info("Sem filtro adicional de classe processual.")
            ...
            with st.spinner("Consultando API do DataJud para todas as UFs..."):
                resultados, erros = consultar_todas_ufs(selected_class)
    ...
    **Sobre a Consulta de Prescrição**
    
    Esta aplicação busca especificamente processos com status "Arquivado - Extinção da Punibilidade - Prescrição",
